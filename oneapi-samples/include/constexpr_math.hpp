...
template <typename T> constexpr T RoundUpToMultiple(T num, T multiple) {
...
// returns n^2
template <typename T> constexpr T Pow2(T n) {
  static_assert(std::is_integral_v<T>);
  return (n < 0) ? (T(1) << (-n)) : (T(1) << n);
}

// returns whether abs(n) is a power of 2
template <typename T> constexpr bool IsPow2(T n) {
  static_assert(std::is_integral_v<T>);
  if (n < 0)
    n = -n;
  return (n != 0) && ((n & (n - 1)) == 0);
...
template <typename T> static constexpr T CeilLog2(T n) {
  return ((n == 1) ? T(0) : Log2(n - 1) + T(1));
}

// returns the number of bits required to encode all the values between 0 and N
template <unsigned int n> static constexpr unsigned int BitsForMaxValue() {
  return CeilLog2(n + 1);
}

// return 'n' rounded up to the nearest power of 2
template <typename T> constexpr T RoundUpPow2(T n) {
  static_assert(std::is_integral_v<T>);
  static_assert(std::is_unsigned_v<T>);
  if (n == 0) {
    return 2;
